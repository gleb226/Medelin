...
class LocationDatabase:
    ...
    async def add_location(self, name, address, schedule, google_maps_url, coordinates=None, image_url='', amenities=None, atmosphere=''):
        db = await get_db()
        res = await db.locations.insert_one({'name': name, 'address': address, 'schedule': schedule, 'google_maps_url': google_maps_url, 'coordinates': coordinates or {'lat': 0.0, 'lon': 0.0}, 'image_url': image_url or '', 'amenities': amenities or [], 'atmosphere': atmosphere or ''})
    ...
    async def get_all_locations(self):
        ...
        locs = await cursor.to_list(length=None)
