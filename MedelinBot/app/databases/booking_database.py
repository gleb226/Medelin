...
class BookingDatabase:
    ...
    async def add_booking(self, user_id, username, fullname, phone, location_id, date_time, people_count, wishes, cart, order_type='booking', table_number=''):
        ...
        doc = {'_id': oid, 'id': str(oid), 'user_id': int(user_id) if user_id is not None else None, 'username': username, 'fullname': fullname, 'phone': phone, 'phone_digits': digits, 'location_id': str(location_id), 'date_time': date_time, 'people_count': people_count, 'wishes': wishes, 'cart': cart, 'status': 'new', 'order_type': order_type, 'table_number': table_number or '', 'payment_id': None, 'provider_payment_id': None, 'notified_admin_ids': [], 'refund_status': None, 'created_at': datetime.utcnow()}
    ...
    async def get_new_bookings_by_locations(self, location_ids):
        ...
        cur = db.bookings.find({'status': 'new', 'location_id': {'$in': [str(x) for x in location_ids]}}).sort('created_at', -1)
    ...
    async def set_payment_id(self, booking_id: str, payment_id: str, provider_payment_id: str | None=None):
        ...
        await db.bookings.update_one({'_id': oid}, {'$set': {'payment_id': payment_id, 'provider_payment_id': provider_payment_id}})
    ...
    async def set_refund_status(self, booking_id: str, status: str):
        ...
        await db.bookings.update_one({'_id': oid}, {'$set': {'refund_status': str(status)}})
    ...
    async def mark_admin_notified(self, booking_id: str, admin_id: int):
        ...
        await db.bookings.update_one({'_id': oid}, {'$addToSet': {'notified_admin_ids': int(admin_id)}})
    ...
    async def get_unnotified_new_bookings_for_admin(self, admin_id: int, location_ids: list):
        ...
        cur = db.bookings.find({'status': 'new', 'location_id': {'$in': [str(x) for x in location_ids]}, 'notified_admin_ids': {'$ne': int(admin_id)}}).sort('created_at', -1)
    ...
    async def get_user_by_phone(self, phone: str) -> Any:
        ...
        return await db.bookings.find_one({'phone_digits': t}, {'_id': 0, 'user_id': 1, 'fullname': 1, 'username': 1, 'phone': 1}, sort=[('created_at', -1)])
