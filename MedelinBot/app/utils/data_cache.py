        ...
        for cat in categories:
            ...
            for i in items:
                ...
                try:
                    ...
                    if use_default:
                        # Ми фільтруємо, якщо напій містить назву молока/сиропу, які тепер є опціями
                        restricted_words = ['бананов', 'ванільн', 'полуничн', 'шоколадн', 'кокосов', 'мигдалев', 'вівсян', 'крісп']
                        if any(x in name_l for x in restricted_words):
                            # Винятки для напоїв, які мають бути в основному списку (назви самих категорій або унікальні напої)
                            exceptions = ['какао', 'матча лате', 'матча тонік', 'айс лате', 'айс-лате', 'глясе', 'раф', 'кумис']
                            if not any(exc in name_l for exc in exceptions):
                                # Якщо це напій з молоком у назві, і він не є винятком - пропускаємо його (він тепер в опціях)
                                if any(x in name_l for x in ['лате', 'латте', 'капуч', 'матча', 'какао', 'мілк']):
                                    continue
