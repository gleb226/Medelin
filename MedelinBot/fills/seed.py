...
MENU_OVERRIDES = [
    ...
    ('Кава', 'Глясе', '73', 'Прохолодна кава with великою кулькою ніжного ванільного морозива.', '250 мл', '180 ккал', '2', 'висока', '100% арабіка + морозиво', True, True, ('Бразилія', '1100м', 'Арабіка', 'Натуральна', 'Середнє', 'Горіх, шоколад')),
...
MENU_SECTIONS = [
    ('Кава', [('Американо', 43), ('Глясе', 73), ('Допіо', 84), ('Еспресо', 42), ('Еспресо Макіато', 45), ('Еспресо тонік', 85), ('Капучіно на подвійному', 75), ('Капучино', 57), ('Кортадо', 52), ('Лате', 63), ('Лате на подвійному', 85), ('Раф', 76), ('Фільтр', 60), ('Флет вайт', 94), ('Рістрето', 42), ('Айс лате', 63)]),
    ('Десерти', [('Вишневий чіз', 110), ('Горіх-лате чіз', 110), ('Горішки', 30), ('Десерт в асортименті', 110), ('Еклер карамель', 80), ('Шу малиновий', 95), ('Брауні', 65), ('Веган малиновий байт', 125), ('Веган тарт снікерс', 125), ('Еклер фісташка', 80), ('Кремова картопля', 75), ('Макаронс', 70), ('Наполеон', 110), ('Чізкейк бакський', 110), ('Трубочка фісташкова', 78), ('Цукерки веган', 52), ('Батончик фундук-кава', 90), ('Канелє', 45)]),
    ('Напої', [('Бейбічіно', 30), ('Вода з лимоном', 15), ('Джміль', 105), ('Лимонад', 90), ('Сік коник', 60), ('Сік березовий', 60), ('Деренівська газ', 60), ('Деренівська не газ', 60), ('Вода з лимоном 0,5л', 30)]),
    ('Масала', [('Масала', 90)]),
    ('Фреш', [('Апельсиновий фреш', 100), ('Апельсиново-грейпфрутовий фреш', 100), ('Яблучний фреш', 70), ('Яблучно-морквяний фреш', 70)]),
    ('Чай', [('Зелений чай', 70), ("Трав'яний чай", 70), ('Фруктовий чай', 70), ('Чай лохина', 70), ('Чорний чай', 70)]),
    ('Мілк', [('Банановий мілкшейк', 85), ('Класичний мілкшейк', 80), ('Полуничний мілкшейк', 85), ('Шоколадний мілкшейк', 85), ('Фрапе', 93)]),
    ('Какао', [('Какао', 52), ('Какао з маршмелоу', 65)])
]
...
def _guess_volume(category: str, name: str) -> str:
    ...
    c = _norm(category)
    if c == 'десерти':
        if 'макарон' in n: return '70 г'
        if 'горіш' in n: return '50 г'
        return '150 г'
    if c == 'кава':
        if 'рістр' in n or 'еспресо' in n: return '30 мл'
        if 'допіо' in n: return '60 мл'
        if 'америк' in n: return '150 мл'
        if 'капуч' in n or 'флет' in n or 'корт' in n: return '200 мл'
        if 'лате' in n: return '300 мл'
        if 'глясе' in n: return '250 мл'
        return '250 мл'
    if c in {'чай', 'фреш'}: return '400 мл' if c == 'чай' else '300 мл'
    if c in {'матча', 'какао', 'мілк', 'масала'}: return '250 мл'
    return '300 мл'
...
def _guess_calories(category: str, name: str) -> str:
    ...
    c = _norm(category)
    if c == 'кава':
        if 'рістр' in n or ('еспресо' in n and 'тонік' not in n): return '5 ккал'
        if 'америк' in n or 'фільтр' in n: return '5 ккал'
        if 'глясе' in n: return '180 ккал'
        if 'лате' in n: return '150 ккал'
        if 'капуч' in n or 'флет' in n or 'корт' in n: return '120 ккал'
        if 'раф' in n: return '250 ккал'
        if 'тонік' in n: return '90 ккал'
        return '120 ккал'
    if c == 'десерти':
        if 'браун' in n: return '420 ккал'
        if 'еклер' in n: return '290 ккал'
        if 'чіз' in n or 'чізкейк' in n: return '380 ккал'
        return '320 ккал'
    if c in {'матча', 'какао', 'мілк', 'масала'}: return '200 ккал'
    if c in {'фреш', 'напої'}: return '120 ккал'
    if c == 'чай': return '2 ккал'
    return '100 ккал'
...
def _guess_strength(category: str, name: str) -> str:
    ...
    c = _norm(category)
    if c == 'кава':
        if 'рістр' in n or 'допіо' in n: return '5'
        if 'еспресо' in n: return '4'
        if 'америк' in n or 'фільтр' in n: return '3'
        if 'флет' in n: return '4'
        if 'капуч' in n or 'корт' in n: return '3'
        if 'лате' in n or 'глясе' in n or 'раф' in n: return '2'
        return '3'
    if c == 'чай': return '2'
    if c in {'матча', 'масала'}: return '2'
    return '1'
...
def _guess_sweetness(category: str, name: str) -> str:
    ...
    c = _norm(category)
    if c == 'кава':
        if 'глясе' in n or 'раф' in n: return 'середня'
        return 'низька'
    if c == 'десерти': return 'висока'
    if c in {'матча', 'какао', 'мілк', 'масала'}: return 'середня'
    if c in {'фреш'}: return 'середня'
    if c == 'чай': return 'відсутня'
    return 'середня'
...
def _guess_composition(category: str, name: str) -> str:
    ...
    c = _norm(category)
    if c == 'кава':
        if 'тонік' in n: return 'Еспресо, тонік, лід, лимон'
        if 'америк' in n: return 'Еспресо, вода'
        if 'еспресо' in n or 'рістр' in n or 'допіо' in n: return 'Еспресо'
        if 'фільтр' in n: return 'Фільтр-кава'
        if 'глясе' in n: return 'Еспресо, морозиво, лід'
        return 'Еспресо, молоко'
    if c == 'десерти': return 'Фірмовий десерт'
    if c == 'чай': return "Листовий чай / трав'яний збір"
    if c == 'фреш': return 'Натуральний фреш'
    if c == 'какао': return 'Какао-порошок, молоко'
    if c == 'матча': return 'Матча, молоко'
    if c == 'мілк': return 'Молоко, наповнювач'
    if c == 'масала': return 'Чай, молоко, спеції'
    return 'Фірмовий напій'

def _guess_description(category: str, name: str) -> str:
    c = _norm(category)
    if c == 'кава': return 'Фірмова кава Medelin зі свіжим обсмаженням.'
    if c == 'десерти': return 'Десерт з асортименту Medelin. Деталі уточнюй у бариста.'
...
def _build_menu_data() -> list[tuple]:
    ...
    for item in MENU_OVERRIDES:
        ...
        if _norm(cat) == 'матча': continue
        overrides[(str(cat), _norm(name))] = item
    
    aliases = {
        ('Кава', 'фільтр'): ('Кава', 'фільтр кава'),
        ('Кава', 'флет вайт'): ('Кава', 'флет вайт'),
        ('Кава', 'глясе'): ('Кава', 'глясе'),
        ('Напої', 'лимонад'): ('Напої', 'лимонад класичний')
    }
    
    menu = []
    for category, items in MENU_SECTIONS:
        if _norm(category) == 'матча': continue
        for raw_name, price in items:
            name = str(raw_name).strip()
            key = (str(category), _norm(name))
            src = overrides.get(key)
            if not src and key in aliases: src = overrides.get(aliases[key])
...
BEANS_BASE = [
    {'name': 'Індія монсун малабар', 'price_250': 271},
    {'name': 'Індія плантейшн', 'price_250': 245},
    {'name': 'Італьяно', 'price_250': 210},
    {'name': 'Ефіопія йергачіф', 'price_250': 281},
    {'name': 'Ефіопія сідамо', 'price_250': 282},
    {'name': 'Кенія кіліманджаро', 'price_250': 322},
    {'name': 'Колумбія супремо', 'price_250': 261},
    {'name': 'Мастермікс', 'price_250': 218},
    {'name': 'Мексика марагоджип', 'price_250': 348},
    {'name': 'Преміум', 'price_250': 228},
    {'name': 'Колумбія без кофеїну', 'price_250': 271},
    {'name': 'Нікарагуа', 'price_250': 253}
]

def _build_beans_data() -> list[dict]:
    overrides = {_norm(b.get('name', '')): b for b in BEANS_OVERRIDES or []}

    def make_default(bean_name: str, price_250: int) -> dict:
        return {
            'name': bean_name,
            'price_250': int(price_250),
            'sort': 'Арабіка 100%',
            'roast': 'Середнє',
            'taste': 'Шоколад, горіх, карамель',
            ...
            'cup_score': '',
            'harvest': '',
            'recommendation': 'Підійде для еспресо та альтернативи.',
            'region': '',
            'station': ''
        }
    ...
    for b in BEANS_BASE:
        ...
        if key in overrides:
            ...
            beans.append(make_default(name, int(b.get('price_250') or 0)))
            seen.add(key)
...
async def seed():
    ...
    for item in MENU_DATA:
        ...
        await menu_db.add_item(
            category=cat, name=name, price=price, description=desc, volume=vol, calories=cal, 
            image_url=PHOTO_URL_MENU, strength=STR_MAP.get(strng, 0), sweetness=SWEET_MAP.get(sweet, 0), 
            composition=comp, options=opts, country=c_country, altitude=c_altitude, sort=c_sort, 
            processing=c_proc, roast=c_roast, taste=c_taste
        )
